...
if __name__ == "__main__":
    ...
    im_src = Image.open('gta.png').convert('RGB')
    im_trg = Image.open('cs.png').convert('RGB')
    ...
    if src_in_trg.dtype != np.uint8:
      src_in_trg = np.clip(src_in_trg, 0, 255)  # Clip per assicurarsi che i valori siano tra 0 e 255
      src_in_trg = src_in_trg.astype(np.uint8)

    img = Image.fromarray(src_in_trg)
    img.save(f"./FDA_test.png")